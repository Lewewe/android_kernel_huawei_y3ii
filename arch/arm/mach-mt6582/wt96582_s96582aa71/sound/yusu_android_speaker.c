...
#define PRINTK(format, args...) printk("[yusu]" format,##args )
#else
...
void Sound_Speaker_Turnon(int channel)
{
	...
	for (i = 0; i < AW8736_MODE; i++) {
		mt_set_gpio_out(GPIO_SPK_AMP_EN, 0);
		mt_set_gpio_out(GPIO_SPK_AMP_EN, 1);
	}
#elif defined(AMP_CLASS_D)
...
{
	PRINTK("Audio_eamp_command type is %d\n", type);
	...
	if (type == EAMP_HEADPHONE_OPEN) {
		mt_set_gpio_out(GPIO_HP_AMP_EN, 1);
	} else if (type == EAMP_HEADPHONE_CLOSE) {
		mt_set_gpio_out(GPIO_HP_AMP_EN, 0);
	}
	return 0;
}
