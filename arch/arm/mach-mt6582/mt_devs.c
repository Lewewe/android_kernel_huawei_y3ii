...
extern char emmc_device_name[30];
#ifdef CONFIG_MTK_EXTMEM
static phys_addr_t lca_reserved_mem_start;
static size_t lca_reserved_mem_size;

phys_addr_t get_lca_reserved_mem_start(void)
{
	return lca_reserved_mem_start;	
}

size_t get_lca_reserved_mem_size(void)
{
	return lca_reserved_mem_size;	
}
#endif
...
void mt_fixup(struct tag *tags, char **cmdline, struct meminfo *mi)
{
    ...
    for (; tags->hdr.size; tags = tag_next(tags)) {
        ...
        else if(tags->hdr.tag == ATAG_MEM_DRAM_INFO ){
            dram_info = (dram_info_t*)(&tags->u.dram_info);
        ...
        else if(tags->hdr.tag == ATAG_MEM_MBLOCK_INFO){
            mblock_info = (mblock_info_t *)(&tags->u.mblock_info);
            if (mblock_info) {
        ...
        else if(tags->hdr.tag == ATAG_MEM_LCA_DESC){
            /* lca reserved memory */
            mem_desc = (mem_desc_t *)(&tags->u.lca_reserved_mem);
            if (mem_desc && mem_desc->size) {
            #ifdef CONFIG_MTK_EXTMEM
				lca_reserved_mem_start = mem_desc->start;
				lca_reserved_mem_size = mem_desc->size;
			#endif
                MTK_MEMCFG_LOG_AND_PRINTK(KERN_ALERT
        ...
        else if(tags->hdr.tag == ATAG_MEM_TEE_DESC){
            /* tee reserved memory */
            mem_desc = (mem_desc_t *)(&tags->u.tee_reserved_mem);
            if (mem_desc && mem_desc->size) {
                MTK_MEMCFG_LOG_AND_PRINTK(KERN_ALERT
