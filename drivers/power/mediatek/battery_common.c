...
kal_bool isrunin_test_now = KAL_FALSE;
#define MAX_TRY_TIMES 5
#define RUNIN_BUF_SIZE 4
#define RUNIN_FILE_PATH "/data/anr/runinflag.bat"

static void isrunin_test_state(void)
{
	static int check_runin_state_time = 0;
	struct file *pfile = NULL;
	mm_segment_t old_fs;
	loff_t pos = 0;
	char *buf_rw = NULL;
	char *runin_state_buf1 = "start", *runin_state_buf2 = "stop";
	int ret = 0;

	old_fs = get_fs();
	set_fs(KERNEL_DS);

	if ((check_runin_state_time <= MAX_TRY_TIMES) && (isrunin_test_now == KAL_FALSE)) {
		//pfile = filp_open(RUNIN_FILE_PATH, O_RDONLY|O_CREAT, 0666);
		pfile = filp_open(RUNIN_FILE_PATH, O_RDONLY, 0);
		if (IS_ERR(pfile)) {
			isrunin_test_now = KAL_FALSE;
			printk("[isrunin_test_state] open runin state file = %d check time %d with errno = %ld \n",
				isrunin_test_now, check_runin_state_time, PTR_ERR(pfile));
			check_runin_state_time++;
			goto EXIT;
		} else {
			buf_rw = kmalloc(RUNIN_BUF_SIZE, GFP_KERNEL);
			memset(buf_rw, 0xFF, RUNIN_BUF_SIZE); //memset
			/*//for test
			vfs_llseek(pfile, 0, 0);
			pos = 0;
			ret = vfs_write(pfile, runin_state_buf2, strlen(runin_state_buf2), &pos);//test
			printk("[isrunin_test_state] write copy state = %s, size = %d, ret=%d\n", runin_state_buf2, strlen(runin_state_buf2), ret);

			mdelay(1000);
			*/
			vfs_llseek(pfile, 0, 0);
			pos = 0;
			ret = vfs_read(pfile, buf_rw, RUNIN_BUF_SIZE, &pos);
			printk("[isrunin_test_state] read ret=%d\n", ret);
			if (buf_rw != NULL) {
				ret = strncmp(buf_rw, runin_state_buf1, RUNIN_BUF_SIZE);//strlen(runin_state_buf1)
				if(ret == 0){
					isrunin_test_now = KAL_TRUE;
					printk("[isrunin_test_state] [start] read runin state file = %d check time %d state = %s\n",
						isrunin_test_now, check_runin_state_time, buf_rw);
					check_runin_state_time = MAX_TRY_TIMES + 1;
					kfree(buf_rw);
					filp_close(pfile, NULL);
					goto EXIT;
				}
				ret = strncmp(buf_rw, runin_state_buf2, RUNIN_BUF_SIZE);
				if(ret == 0){
					isrunin_test_now = KAL_FALSE;
					printk("[isrunin_test_state] [stop] read runin state file = %d check time %d state = %s\n",
						isrunin_test_now, check_runin_state_time, buf_rw);
					check_runin_state_time = MAX_TRY_TIMES + 1;
					kfree(buf_rw);
					filp_close(pfile, NULL);
				...
				//other word of state
				printk("[isrunin_test_state] [other] read state file faile, state = %s\n", buf_rw);
				check_runin_state_time = MAX_TRY_TIMES + 1;
				kfree(buf_rw);
				filp_close(pfile, NULL);
				goto EXIT;
			} else {
				printk("[isrunin_test_state] read state file buf_read = NULL\n");
				check_runin_state_time = MAX_TRY_TIMES + 1;
				kfree(buf_rw);
				goto EXIT;
			}
...
static ssize_t show_StartRunin_Test(struct device *dev,struct device_attribute *attr, char *buf)
{
	isrunin_test_now = KAL_TRUE;
	battery_log(BAT_LOG_CRTI, "[show_StartRunin_Test] : isrunin_test_now = %d\n", isrunin_test_now);
	return sprintf(buf, "isrunin_test_now=%d\n", isrunin_test_now);
}
static ssize_t store_StartRunin_Test(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	battery_log(BAT_LOG_CRTI, "[store_StartRunin_Test] Not Support Write Function\n");
	return size;
}

static DEVICE_ATTR(StartRunin_Test, 0664, show_StartRunin_Test, store_StartRunin_Test);

static ssize_t show_StopRunin_Test(struct device *dev,struct device_attribute *attr, char *buf)
{
	isrunin_test_now = KAL_FALSE;
	battery_log(BAT_LOG_CRTI, "[show_StopRunin_Test] : isrunin_test_now = %d\n", isrunin_test_now);
	return sprintf(buf, "isrunin_test_now = %d\n", isrunin_test_now);
}
static ssize_t store_StopRunin_Test(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	battery_log(BAT_LOG_CRTI, "[store_StopRunin_Test] Not Support Write Function\n");
	return size;
}

static DEVICE_ATTR(StopRunin_Test, 0664, show_StopRunin_Test, store_StopRunin_Test);

static ssize_t show_IsRunin_Test(struct device *dev,struct device_attribute *attr, char *buf)
{
	battery_log(BAT_LOG_CRTI, "[show_IsRunin_Test] : isrunin_test_now = %d\n", isrunin_test_now);
	return sprintf(buf, "isrunin_test_now = %d\n", isrunin_test_now);
}
static ssize_t store_IsRunin_Test(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	kal_int32 temp;
	if (1 == sscanf(buf, "%d", &temp)) {
		if (temp != 0) {
			isrunin_test_now = KAL_TRUE;
			battery_log(BAT_LOG_CRTI, "[store_IsRunin_Test] : isrunin_test_now = %d\n", temp);
		} else {
			isrunin_test_now = KAL_FALSE;
			battery_log(BAT_LOG_CRTI, "[store_IsRunin_Test] : isrunin_test_now = %d\n", temp);
		}
	} else {
		battery_log(BAT_LOG_CRTI, "[store_IsRunin_Test] format error!\n");
	}
	return size;
}
static DEVICE_ATTR(IsRunin_Test, 0664, show_IsRunin_Test, store_IsRunin_Test);
...
static kal_bool mt_battery_100Percent_tracking_check(void)
{
	...
	if (BMT_status.bat_full == KAL_TRUE)	/* charging full first, UI tracking to 100% */
	{
		if (BMT_status.UI_SOC >= 100) {
			...
			if((BMT_status.temperature >= 0)&&(BMT_status.temperature <= 45)&&(isrunin_test_now == KAL_FALSE))
#endif
			{
...
static kal_bool mt_battery_0Percent_tracking_check(void)
{
	...
	battery_log(BAT_LOG_CRTI, "0Percent, VBAT=%d SYSTEM_OFF_VOLTAGE=%d UI_SOC=%d\n",
		BMT_status.bat_vol, SYSTEM_OFF_VOLTAGE, BMT_status.UI_SOC);
...
static kal_bool mt_battery_0Percent_tracking_check_in_charging(void)
{
	...
	battery_xlog_printk(BAT_LOG_CRTI,
					"[Battery] mt_battery_0Percent_tracking_check_in_charging(), VBAT < %d UI_SOC = (%d)\r\n",
...
static kal_bool mt_vbat_soc_n_percent_tracking(void)
{
	...
	if(BMT_status.UI_SOC <(vbat_checking_point_SOC+VBAT_LOW_PERCENT_TRACKING_THRESHHOLD) ) {
		if((BMT_status.bat_vol <=vbat_checking_point)&&(BMT_status.UI_SOC>vbat_checking_point_SOC)) {
			...
			battery_xlog_printk(BAT_LOG_CRTI,
							"[ vbat nPercent] vbat %d <= vbat_checking_point %d, UI_SOC=%d., vbat_checking_point_SOC=%d \n",
			...
			battery_xlog_printk(BAT_LOG_CRTI,
							" other  mt_vbat_soc_n_percent_tracking !!\n");
		}
...
static void battery_update(struct battery_data *bat_data)
{
...
#ifdef WT_0PERCENT_CHECK_IN_CHARGING
	int uisoc_jump_in_charging = 0;
#endif

#ifdef WT_BATTERY_CAPACITY_INFO

	if ((g_fg_battery_id == 0) || (g_fg_battery_id == 1))
		bat_data->charge_full_design = 2100 * 1000;
	else if ((g_fg_battery_id == 2) || (g_fg_battery_id == 3))
		bat_data->charge_full_design = 2150 * 1000;
	else
		bat_data->charge_full_design = 2100 * 1000;
	...
	if ((BMT_status.charger_exist == KAL_TRUE) && (BMT_status.bat_charging_state != CHR_ERROR)) {
		if (BMT_status.bat_exist) {	/* charging */
			...
			if (BMT_status.UI_SOC >= BMT_status.SOC)
			{
				if (isrunin_test_now == KAL_FALSE) {
					// set uisoc_jump_in_charging to 30% in normal mode
					uisoc_jump_in_charging = 30;
				} else {
					// set uisoc_jump_in_charging to 15% in runin test
					uisoc_jump_in_charging = 15;
				}

				if (((BMT_status.UI_SOC - BMT_status.SOC) >= uisoc_jump_in_charging) && (BMT_status.UI_SOC != 100)) {
					BMT_status.UI_SOC = BMT_status.SOC;
					resetBatteryMeter = KAL_TRUE;
				}
				if (BMT_status.bat_vol <= V_0PERCENT_TRACKING_in_charging) {
					if(g_boot_mode == NORMAL_BOOT) {
						resetBatteryMeter = mt_battery_0Percent_tracking_check_in_charging();
						battery_xlog_printk(BAT_LOG_CRTI,
							"[Battery] mt_battery_0Percent_tracking_check_in_charging(), VBAT < %d BMT_status.bat_vol = (%d)\r\n",
...
static void mt_battery_notify_high_temp_charging_full_warning_check(void)
{
	...
	if (g_BatteryNotifyCode != 0x0000)
		battery_xlog_printk(BAT_LOG_CRTI,
			"[BATTERY] mt_battery_notify_high_temp_charging_full_warning_check (%x)\n",
			g_BatteryNotifyCode);
}
#endif
...
#ifdef WT_0PERCENT_CHECK_IN_CHARGING
static kal_bool is_notify_warning = KAL_FALSE;
static void mt_battery_notify_low_vbat_in_charging_mode_check(void)
{
	if ((BMT_status.UI_SOC == 0) && \
		...
		if (BMT_status.bat_vol <= 3300) {
			if (is_notify_warning == KAL_TRUE) {
				g_BatteryNotifyCode |= 0x0100; //notify power off
			} else {
				//g_BatteryNotifyCode |= 0x0080; //notify warning
				//g_BatteryNotifyCode |= 0x0100; //notify power off
				g_BatteryNotifyCode |= 0x0200;
			}
			battery_log(BAT_LOG_CRTI,
				"[BATTERY] notify power off BMT_status.bat_vol = %d mV\n",
				BMT_status.bat_vol);
		} else if ((BMT_status.bat_vol > 3300) && (BMT_status.bat_vol < 3400)) {
			g_BatteryNotifyCode |= 0x0080; //notify warning
			is_notify_warning = KAL_TRUE;
			battery_log(BAT_LOG_CRTI,
				"[BATTERY] notify warning BMT_status.bat_vol = %d mV\n",
				BMT_status.bat_vol);
		} else {
			g_BatteryNotifyCode &= ~(0x0080);
			g_BatteryNotifyCode &= ~(0x0100);
			is_notify_warning = KAL_FALSE;
		}

	}
	if (g_BatteryNotifyCode != 0x0000)
		battery_log(BAT_LOG_CRTI,
			"[BATTERY] mt_battery_notify_low_vbat_in_charging_mode_check (%x)\n",
			g_BatteryNotifyCode);
}
#endif
...
#ifdef WT_0PERCENT_CHECK_IN_CHARGING
		mt_battery_notify_low_vbat_in_charging_mode_check();
#endif
...
static int nonstandard_check_times = 0;
#define NONSTANDARD_CHARGER_CHECK_TIMES 3
CHARGER_TYPE mt_charger_type_detection(void)
{
	...
	#endif
	{
		...
		if (BMT_status.charger_type == NONSTANDARD_CHARGER) {
			if (nonstandard_check_times > NONSTANDARD_CHARGER_CHECK_TIMES) {
				mutex_unlock(&charger_type_mutex);
				return BMT_status.charger_type;
			} else {
				battery_log(BAT_LOG_CRTI,
						"[BAT_thread]Cable in, CHR_Type_num=%d, needs to recheck, time %d\n",
						BMT_status.charger_type, nonstandard_check_times);
				nonstandard_check_times++;
			}
		} else {
			nonstandard_check_times = 0;
			battery_log(BAT_LOG_CRTI,
					"[BAT_thread]Cable in, CHR_Type_num=%d, reset nonstandard_check_times %d\n",
					BMT_status.charger_type, nonstandard_check_times);
		}
		battery_charging_control(CHARGING_CMD_GET_CHARGER_TYPE, &CHR_Type_num);
		BMT_status.charger_type = CHR_Type_num;
...
int do_check_for_cdp = 0; //for count cdp check times
//add end
...
void do_chrdet_int_task(void)
{
	if (g_bat_init_flag == KAL_TRUE) {
		...
		#endif
		{
			...
			BMT_status.charger_exist = KAL_TRUE;
			nonstandard_check_times = 0; //reset nonstandard_check_times when charger exist
			...
			if (g_platform_boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT
				...
				if (do_check_for_cdp < 3) {
					battery_log(BAT_LOG_CRTI,
						"[pmic_thread_kthread] count do_check_for_cdp = %d\n", (do_check_for_cdp+1));
					do_check_for_cdp++; //count
				} else {
					battery_log(BAT_LOG_CRTI,
...
static int battery_probe(struct platform_device *dev)
{
	...
	/* For EM */
	{
		...
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_IsRunin_Test);
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_StartRunin_Test);
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_StopRunin_Test);
...
static void battery_timer_resume(void)
{
	...
	battery_log(BAT_LOG_CRTI, "battery resume g_call_state is %d\n",g_call_state); 
	battery_log(BAT_LOG_CRTI, "battery resume batt_cust_data.talking_sync_time is %d\n",batt_cust_data.talking_sync_time);
	...
	if((g_call_state == CALL_ACTIVE) && (bat_time_after_sleep.tv_sec - g_bat_time_before_sleep.tv_sec >= batt_cust_data.talking_sync_time))	// phone call last than x min
	{
		...
		if((bat_is_charger_exist() == KAL_FALSE)&&(BMT_status.UI_SOC > battery_meter_get_battery_percentage()))
		{
			BMT_status.UI_SOC = battery_meter_get_battery_percentage();
