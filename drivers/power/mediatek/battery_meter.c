...
void oam_init(void)
{
	...
	{
		bm_print(BM_LOG_CRTI,
				"[oam_init] UbVbat = %d, gFG_voltage = %d, g_booting_vbat = %d\n",
				UbVbat, gFG_voltage, g_booting_vbat);

		if(abs(UbVbat - gFG_voltage) > 30) {
			bm_print(BM_LOG_CRTI,
					"[oam_init] Set hw_ocv(%d) to UbVbat(%d) \n ",
					gFG_voltage, UbVbat);
			gFG_voltage = UbVbat;
		}
		if(abs(UbVbat - g_booting_vbat) > 30) {
			bm_print(BM_LOG_CRTI,
				"[oam_init] Set vbat(%d) to UbVbat(%d) \n ",
				g_booting_vbat, UbVbat);
			g_booting_vbat = UbVbat;
		}
	}
...
kal_int32 get_dynamic_period(int first_use, int first_wakeup_time, int battery_capacity_level)
{
	...
	{
		g_spm_timer=10;
		bm_print(BM_LOG_CRTI, "test by mtk g_spm_timer=%d\n", g_spm_timer);
		return g_spm_timer;
	}
	#endif
...
#ifdef MTK_MULTI_BAT_PROFILE_SUPPORT
static ssize_t show_FG_g_fg_dbg_battery_id(struct device *dev, struct device_attribute *attr,
					 char *buf)
{
	bm_print(BM_LOG_CRTI, "[FG] show_FG_g_fg_dbg_battery_id : %d\n", g_fg_battery_id);
	return sprintf(buf, "%d\n", g_fg_battery_id);
}

static DEVICE_ATTR(FG_g_fg_dbg_battery_id, 0664, show_FG_g_fg_dbg_battery_id, NULL);
#endif
...
#ifdef MTK_MULTI_BAT_PROFILE_SUPPORT
	ret_device_file = device_create_file(&(dev->dev), &dev_attr_FG_g_fg_dbg_battery_id);
#endif
...
static int get_torch_state()
{
	return mt_get_gpio_out(GPIO_CAMERA_FLASH_EN_PIN);
}
...
#endif
	#if 0
	//add by cassy begin 2
	vol_bat1 = 15; //set avg times
	ret = battery_meter_ctrl(BATTERY_METER_CMD_GET_ADC_V_BAT_SENSE, &vol_bat1);
	HW_OCV1 = vol_bat1 + mtk_imp_tracking(vol_bat1, 800, 5); // 80ma can config 
	battery_xlog_printk(BAT_LOG_CRTI, "[cassy] vol_bat1=%d, HW_OCV1=%d\n", vol_bat1 ,HW_OCV1);
	D0=fgauge_read_d_by_v(HW_OCV1);
	battery_xlog_printk(BAT_LOG_CRTI, "[cassy] D0=%d,UI_SOC=%d\n", D0,100-D0); 
	//add by cassy end 2
	#endif
...
#if !defined(FG_SELF_DISCHARGE_CHECK)
	battery_meter_ctrl(BATTERY_METER_CMD_GET_HW_OCV, &hw_ocv_after_sleep);
	bm_print(BM_LOG_CRTI,"[battery_meter_resume] BMT_status.UI_SOC=%d, g_call_state =%d\n", BMT_status.UI_SOC, g_call_state);
	//bm_print(BM_LOG_CRTI,"g_hw_ocv_before_sleep=%d,hw_ocv_after_sleep=%d\n", g_hw_ocv_before_sleep, hw_ocv_after_sleep);

	bm_print(BM_LOG_CRTI, "[battery_meter_resume] g_torch_state = %d\n", get_torch_state());
	if((g_call_state == CALL_ACTIVE) || get_torch_state()) {
		if (_g_bat_sleep_total_time >= g_spm_timer) {	/* 0.5hr */
			if (hw_ocv_after_sleep != g_hw_ocv_before_sleep) {
				#if 1
				oam_d0 = fgauge_read_d_by_v(hw_ocv_after_sleep);
				oam_v_ocv_2 = oam_v_ocv_1 = hw_ocv_after_sleep;
				#else
				oam_d0=fgauge_read_d_by_v(HW_OCV1);
				oam_v_ocv_2 = oam_v_ocv_1 = HW_OCV1;
				#endif
				oam_car_1 = 0;
				oam_car_2 = 0;
				oam_d_2 = oam_d0;//add this line
				oam_d_5 = oam_d0;//add
				bm_print(BM_LOG_CRTI,
					"[battery_meter_resume] call or troch hw_ocv_after_sleep =%d,oam_d0=%d\n",
					hw_ocv_after_sleep, oam_d0);
				if(BMT_status.UI_SOC > (100-oam_d0)) {
					if (g_call_state == CALL_ACTIVE)
						resume_soc_jump = 4; //150~160mA
					else if (get_torch_state())
						resume_soc_jump = 5; //200mA
					else
						resume_soc_jump = 4; //default
					if((BMT_status.UI_SOC - (100-oam_d0)) > resume_soc_jump ) {
						BMT_status.UI_SOC = (BMT_status.UI_SOC - resume_soc_jump);
					} else {
		...
		if (_g_bat_sleep_total_time >= g_spm_timer) {	/* 0.5hr */
			if (hw_ocv_after_sleep < g_hw_ocv_before_sleep) {
				...
				oam_d_5=oam_d0;//add
				bm_print(BM_LOG_CRTI,
					"[battery_meter_resume] normal oam_d0=%d,hw_ocv_after_sleep is %d\n",
					oam_d0, hw_ocv_after_sleep);

				if(BMT_status.UI_SOC > (100-oam_d0))
				{
					if ((BMT_status.UI_SOC - (100-oam_d0)) >= (_g_bat_sleep_total_time/600))
						BMT_status.UI_SOC -= (_g_bat_sleep_total_time/600);
					else
						BMT_status.UI_SOC = (100-oam_d0);
				}
				if (BMT_status.UI_SOC < 0)
