	...
	if (g_ftm_battery_flag) {
		...
		if (BMT_status.charger_type == STANDARD_HOST) {
			...
			if((BMT_status.temperature >= 0)&&(BMT_status.temperature <10)) {
				g_temp_CC_value = AC_CHARGER_CURRENT_NORMAL_TEMP;
			} else {
...
#else
			g_temp_CC_value = AC_CHARGER_CURRENT;
			//g_temp_CC_value = batt_cust_data.ac_charger_current; //disable new set
...
#ifdef WT_UI_SOC_SYNC_FULL_IN_CHARGING
	battery_log(BAT_LOG_CRTI,
					"[BATTERY] Battery check full charging is %d mA, sync to 100 is %d, sync to full is %d \n",
					BMT_status.ICharging, WT_UI_SOC_SYNC_FULL_ITERM, charging_full_current);
	if(BMT_status.ICharging < WT_UI_SOC_SYNC_FULL_ITERM) {
		ui_soc_sync_100_enable = KAL_TRUE;
		//battery_log(BAT_LOG_CRTI, "[BATTERY] charging_full_check  ui_soc_sync_100_enable = KAL_TRUE ! \n");
	}
#endif
		...
		if (g_temp_CC_value == CHARGE_CURRENT_0_00_MA) {
			...
			if(batt_cust_data.high_battery_voltage_support) {
				...
				if((BMT_status.temperature >= 0)&&(BMT_status.temperature <= 45)) {
					cv_voltage = NORMAL_TEMP_CV_VOLTAGE;
				} else if((BMT_status.temperature > 45)&&(BMT_status.temperature <= 60)) {
					cv_voltage = HIGH_TEMP_CV_VOLTAGE;
				} else {
					cv_voltage = NORMAL_TEMP_CV_VOLTAGE;
				}
				#else
			...
			battery_log(BAT_LOG_CRTI,
				"[BATTERY] high_battery_voltage_support = %d, temperature = %d, cv_voltage = %d\n",
				batt_cust_data.high_battery_voltage_support, BMT_status.temperature, cv_voltage);
			battery_charging_control(CHARGING_CMD_SET_CV_VOLTAGE,&cv_voltage);
#endif
#if 0 //disable oringal
#if !defined(CONFIG_MTK_JEITA_STANDARD_SUPPORT)
	if(batt_cust_data.high_battery_voltage_support)
			cv_voltage = BATTERY_VOLT_04_350000_V;
	else
...
PMU_STATUS BAT_PreChargeModeAction(void)
{
	...
	{ 
		if (BMT_status.bat_vol > batt_cust_data.v_pre2cc_thres) 
		{
...
if(batt_cust_data.high_battery_voltage_support)
	...
	if (BMT_status.bat_vol < (v_cc2topoff_threshold - 30)) { //v_cc2topoff_threshold - 0.03V
		BMT_status.bat_charging_state = CHR_CC;
		BMT_status.TOPOFF_charging_time = 0;
	}
	//+Other_huawei_req add end
...
PMU_STATUS BAT_BatteryFullAction(void)
{
	...
	//if (BMT_status.bat_vol < batt_cust_data.recharging_voltage) //disable new set
#endif
