...
static void _DISP_WaitMemWriteDone (void) {
	int i;
	disp_job *job = NULL;
	unsigned int ovl_layer0_mva_addr = OVLGetLayer0Addr();
	update_frm_seq_info(disp_config.session_id, ovl_layer0_mva_addr, 0, 0, FRM_TRIGGER);
	//1. waiting for HW frame done
	disp_path_wait_frame_done();
	unsigned int idx = 0;
	unsigned int wdma0_dst_mva_addr = WDMAGetDstAddress(idx);
	update_frm_seq_info(disp_config.session_id, wdma0_dst_mva_addr, 0, 0, FRM_WDMA0_EOF);
...
static int _DISP_ConfigUpdateKThread(void *data)
{
    ...
    while(1)
    {
        ...
        DISP_DRV_UPD("_DISP_ConfigUpdateKThread wake up and run.\n");
...
static void _DISP_ScrUpdStartCallback(void* pParam)
{
	...
	read_mem_running = 1;
	unsigned int idx = 0;
	unsigned int rdma0_addr = RDMAGetAddress(idx);
	unsigned int primary_session = MAKE_DISP_SESSION(DISP_SESSION_DECOUPLE_MODE, DISP_SESSION_PRIMARY, 0);
	update_frm_seq_info(primary_session, rdma0_addr, 0, 0, FRM_RDMA0_SOF);
#endif
}
...
static void _DISP_ScrUpdEndCallback(void* pParam)
{
	...
	unsigned int rdma0_addr = RDMAGetAddress(idx);
	unsigned int primary_session = MAKE_DISP_SESSION(DISP_SESSION_DECOUPLE_MODE, DISP_SESSION_PRIMARY, 0);
	update_frm_seq_info(primary_session, rdma0_addr, 0, 0, FRM_RDMA0_EOF);
...
void DISP_WaitVSYNC(void)
{
    ...
    vsync_wq_flag = 0;
    if (wait_event_interruptible_timeout(vsync_wq, vsync_wq_flag, HZ/10*2) == 0)
    {
