...
#define VSYNC_MS_TO_NS(x)	(x * 1E6L) // thomas
...
#define GPIO_DSI_TE_EINT_PIN         (GPIO113 | 0x80000000)
#define CUST_EINT_DSI_TE_NUM              113
#define GPIO_DSI_TE_EINT_PIN_M_EINT   GPIO_MODE_00
#define CUST_EINT_DSI_TE_POLARITY	  CUST_EINTF_TRIGGER_FALLING
...
#define GPIO_INCELL_DISP_TE_PWM         (GPIO47 | 0x80000000)
...
void DSI_set_fps_for_PWM(unsigned int fps)
{
	...
	printk("DSI_set_fps_for_PWM fps (%d), widht (%d)\n", fps, width);
...
DSI_STATUS DSI_WaitForDispFinish(void)
{
	long ret = 0;
	ret = wait_event_interruptible_timeout(_dsi_wait_vm_done_queue, !_IsEngineBusy(), HZ/10);
	if (0 == ret) {
		DISP_LOG_PRINT(ANDROID_LOG_WARN, "DSI", " Wait for disp finished timeout\n");
		return DSI_STATUS_ERROR;
	}
	return DSI_STATUS_OK;
}
