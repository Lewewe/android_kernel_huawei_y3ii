    ...
    case MTKFB_SET_MULTIPLE_LAYERS:
    {
        ...
        else 
        {
            ...
            for (i = 0; i < HW_OVERLAY_COUNT; ++i) 
            {
                ...
                if ((layerInfo[i].layer_id < 0) || (layerInfo[i].layer_id >= HW_OVERLAY_COUNT))
                {
                    continue;
                }
                if(is_early_suspended)
...
//#if defined(CONFIG_SINGLE_PANEL_OUTPUT)
		//FIXME: If WFD, ScreenRecord, don't do this
		if(is_early_suspended && config.session_id != MAKE_DISP_SESSION(DISP_SESSION_DECOUPLE_MODE,DISP_SESSION_MEMORY,2))
		{
			int i = 0;
			for(i=0; i<HW_OVERLAY_COUNT; i++)
			{
...
//#endif
	...
	case DISP_IOCTL_PREPARE_OUTPUT_BUFFER:
	{
		...
		MMProfileLogStructure(MTKFB_MMP_Events.PrepareOutput, MMProfileFlagStart, &info, disp_buffer_info);
		if (info.layer_en) {
			if (disp_sync_prepare_buffer(&info) != SYNC_STATUS_OK) {
				info.fence_fd = DISP_INVALID_FENCE_FD; // invalid fd
				info.index = 0;
		...
		MMProfileLogEx(MTKFB_MMP_Events.PrepareOutput, MMProfileFlagEnd, disp_config.session_id, r);
		return (r);
	}
	...
	case DISP_IOCTL_SET_INPUT_BUFFER:
	{
		...
		//MTKFB_INFO(" mtkfb_ioctl():DISP_IOCTL_SET_INPUT_BUFFER\n");
		if (copy_from_user(&layerInfo, (void __user *)arg, sizeof(layerInfo))) {
			MTKFB_WRAN("[FB]: copy_from_user failed!\n");
			return -EFAULT;
		}
	...
	case DISP_IOCTL_SET_OUTPUT_BUFFER:
	{
		...
		//MTKFB_INFO(" mtkfb_ioctl():DISP_IOCTL_SET_OUTPUT_BUFFER\n");
		if (copy_from_user(&layerInfo, (void __user *)arg, sizeof(layerInfo))) {
			MTKFB_WRAN("[FB]: copy_from_user failed!\n");
			return -EFAULT;
		}
...
static void mtkfb_fb_565_to_888(void* fb_va)
{
    ...
    MTKFB_INFO("565_to_888, s = 0x%x, d=0x%x\n", s,d);
    for (j = 0; j < yres; ++ j)
    {
        for(k = 0; k < xres; ++ k)
        {
...
static void mtkfb_shutdown(struct device *pdev)
{
	...
	DISP_PrepareSuspend();
    // Wait for disp finished.
	if (DSI_STATUS_ERROR == DSI_WaitForDispFinish())
...
void mtkfb_clear_lcm(void)
{
	...
	for (i = 0; i < DDP_OVL_LAYER_MUN; i++) {
		if (i == FB_LAYER && isAEEEnabled) {
			MTKFB_INFO("[FB Driver] mtkfb_clear_lcm remain AEE layer un-cleared\n");
		} else {
			job->input[i].layer_id = i;
			job->input[i].layer_enable = 0;
			job->input[i].dirty = 1;
