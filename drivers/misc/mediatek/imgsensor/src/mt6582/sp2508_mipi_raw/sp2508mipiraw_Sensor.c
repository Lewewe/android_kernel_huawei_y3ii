...
KAL_FALSE,KAL_FALSE,KAL_FALSE,36000000,36000000,36000000,1,1,1,16,16,16,SP2508MIPI_PV_LINE_LENGTH_PIXELS,SP2508MIPI_PV_FRAME_LENGTH_LINES,
...
static void SP2508MIPI_Sensor_Init(void)
{
	...
	//add digital gain 1.377x解决高亮发粉问题 20150515
	{
		...
		SP2508MIPI_write_cmos_sensor(0x2f,0x0c); //pll clk 60M            
		SP2508MIPI_write_cmos_sensor(0xfd,0x01);
		SP2508MIPI_write_cmos_sensor(0x03,0x03); //exp time, 3 base
		SP2508MIPI_write_cmos_sensor(0x04,0xa2);
		SP2508MIPI_write_cmos_sensor(0x06,0x10); //vblank
		...
		SP2508MIPI_write_cmos_sensor(0x25,0xf0); //reg dac 2.7v, enable bl_en,vbl 1.28v 原为0xf2 改为0xf0 降低太阳黑子替换电压
		...
		SP2508MIPI_write_cmos_sensor(0x8a,0x44); //pixel bias 1.58uA 原为0x55 改为0x44 降低pixel bias
		SP2508MIPI_write_cmos_sensor(0x8b,0x44); 
...
*
*************************************************************************/
#define CMMCLK_DRIVING 0xF0005b60//csq.add
UINT32 SP2508MIPIGetSensorID(UINT32 *sensorID) 
{
    int  retry = 3; 
	SENSORDB("[SP2508MIPI]enter SP2508MIPIGetSensorID function\n");

    //csq.add set cmmclk driving to 8ma start
    unsigned int read_val = 0;
    read_val = GPIO_RD32(CMMCLK_DRIVING);
    read_val &= ~0x0000F000;
    read_val |= (0x4 << 12);
    GPIO_WR32(CMMCLK_DRIVING, read_val);
    //csq.add set cmmclk driving to 8ma end
...
void SP2508MIPISetFlipMirror(kal_int32 imgMirror)
{
    ...
    switch (imgMirror)
    {
        case IMAGE_NORMAL:
            ...
            break;
        case IMAGE_V_MIRROR:
            SP2508MIPI_write_cmos_sensor(0x3f, 0x02);	//Set flip
            break;
        case IMAGE_H_MIRROR:
            SP2508MIPI_write_cmos_sensor(0x3f, 0x01);	//Set mirror
            break;
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	PreviewSetting();
       SP2508MIPISetFlipMirror(IMAGE_NORMAL);//hanlei
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	SP2508MIPISetFlipMirror(IMAGE_NORMAL);	//add by lishengli 20130614 hanlei
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	SP2508MIPI_Auto_Flicker_mode = KAL_FALSE;    
		...
		spin_unlock(&sp2508_drv_lock);
		SP2508MIPI_set_5M();
		SP2508MIPISetFlipMirror(IMAGE_NORMAL);
		spin_lock(&sp2508_drv_lock);
...
{ 
	SENSORDB("[SP2508MIPI]enter SP2508MIPIGetInfo function\n");
      ...
      pSensorInfo->SensorOutputDataFormat=SENSOR_OUTPUT_FORMAT_RAW_R;//SENSOR_OUTPUT_FORMAT_RAW_B;hanlei
