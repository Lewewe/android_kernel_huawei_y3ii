...
#define OV5648_OTP_FUNCTION
#ifdef OV5648_OTP_FUNCTION
kal_uint16 RG_Ratio_Typical = 0x168; //RG_TYPICAL
kal_uint16 BG_Ratio_Typical = 0x135;//BG_TYPICAL
...
int update_otp()
{
    ...
    SENSORDB("index=%d", otp_index);
    if (i>3) {
        // no valid wb OTP data
        return 1;
    }
    read_otp(otp_index, &current_otp);
    SENSORDB("module_integrator_id=0x%x , lens_id=0x%x, rg_ratio=0x%x, bg_ratio=0x%x, user_data[0]=0x%x, user_data[1]=0x%x, light_rg=0x%x, light_bg=0x%x", 
        current_otp.module_integrator_id, current_otp.lens_id, 
        current_otp.rg_ratio, current_otp.bg_ratio, 
        current_otp.user_data[0], current_otp.user_data[1], 
...
int update_awb_otp()
{
    ...
    SENSORDB("R_gain=0x%x, G_gain=0x%x, B_gain=0x%x", R_gain, G_gain, B_gain);
...
#ifdef OV5648_OTP_FUNCTION
    OV5648MIPI_write_cmos_sensor(0x5001, 0x01); // awb disable
#else
...
UINT32 OV5648GetSensorID(UINT32 *sensorID) 
{
    ...
    *sensorID=((OV5648MIPI_read_cmos_sensor(0x300A) << 8) | OV5648MIPI_read_cmos_sensor(0x300B));   
    ...
    OV5648MIPI_Sensor_Init();
    update_otp();
    if (current_otp.module_integrator_id != 0x9) {
        SENSORDB("module id: 0x%x, is not sunrise sensor", current_otp.module_integrator_id);
        // if Sensor ID is not correct, Must set *sensorID to 0xFFFFFFFF 
        *sensorID = 0xFFFFFFFF;
        return ERROR_SENSOR_CONNECT_FAIL;
    }
