...
static int ap3xx6_set_plthres(struct i2c_client *client, int val)
{
	...
	err = ap3xx6_write_reg(client, 0x2A, 0xFF, 0x00, lsb);
	if (err <= 0)
		return err;

	err = ap3xx6_write_reg(client, 0x2B, 0xFF, 0x00, msb);
...
static int ap3xx6_set_phthres(struct i2c_client *client, int val)
{
	...
	err = ap3xx6_write_reg(client, 0x2C, 0xFF, 0x00, lsb);
	if (err <= 0)
		return err;

	err = ap3xx6_write_reg(client, 0x2D, 0xFF, 0x00, msb);
...
static int ap3xx6_set_pcrosstalk(struct i2c_client *client, int val)
{
	...
	lsb = val & 0x01;
#endif
	err = ap3xx6_write_reg(client, 0x28, 0xFF, 0x00, lsb);
	if (err <= 0)
		return err;
	err = ap3xx6_write_reg(client, 0x29, 0xFF, 0x00, msb);
...
/*val=0x00~0x3f*/
	re_val = val&0x3F;
	err = ap3xx6_write_reg(client, 0x26, 0xFF, 0x00, re_val);
#else
/*val=0x00~0x03*/
	re_val = ap3xx6_read_reg(client, 0x20, 0xFF, 0x00);
...
/*val=0x00~0xff*/
	re_val = val&0xFF;
	err = ap3xx6_write_reg(client, 0x06, 0xFF, 0x00, re_val);
#else
/*val=0x00~0x3f*/
	re_val = val&0x3F;
...
//wt add for ps calibration start
static int ap3xx6_read_data_for_cali(struct i2c_client *client, struct PS_CALI_DATA_STRUCT *ps_data_cali)
{
	...
	mdelay(100);//mdelay(5);fangliang
	 if(!ap3xx6_read_ps(client,&data[i]))
	  {
	sum += data[i];
	 }
	else
	...
	if(i == 20)
            {
	data_cali = sum/20;
	 printk("ap3xx6_read_data_for_cali data_cali = %d\n",data_cali);
	 ps_data_cali->far_away = data_cali + CUST_AP_PS_THRES_FAR;
	ps_data_cali->close = data_cali + CUST_AP_PS_THRES_CLOSE;
	 ps_data_cali->valid = 1;

	 if(ps_data_cali->close > 1023)
	 {
	 	ps_data_cali->close = 1023;
	 }
	...
	APS_LOG("ap3xx6_read_data_for_cali get data error!\n");
	err=  -1;
	 }
	return err;
	}
static void ap3xx6_WriteCalibration(struct i2c_client *client, struct PS_CALI_DATA_STRUCT *data_cali)
{
		 struct ap3xx6_priv *obj = i2c_get_clientdata(client);
...
static long ap3xx6_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case ALSPS_SET_PS_CALI:
		...
		ap3xx6_WriteCalibration(obj->client,&ps_cali_temp);
	...
	case ALSPS_GET_PS_RAW_DATA_FOR_CALI:
		if((err = ap3xx6_enable_ps(obj->client, 1))!=0)
		{
			APS_ERR("disable ps fail: %ld\n", err);
			return -1;
		}
		ap3xx6_read_data_for_cali(obj->client,&ps_cali_temp);
...
*/
	...
	*value = obj->als;   
    ...
    if(ir_value < 200)    //cwf and office light
    {
        ...
        *value =(obj->als)*748/1000;  
        *status = SENSOR_STATUS_ACCURACY_MEDIUM;
        printk("---CW and Office---als_value_lux = %d,ir_value = %d\n",obj->als,ir_value);
    }
    else   //D65 and A light
    {
        *value = (obj->als)*329/1000;   
        *status = SENSOR_STATUS_ACCURACY_MEDIUM;
        printk("---D65 and A---als_value_lux = %d,ir_value = %d\n",obj->als,ir_value);
    }
...
#include <linux/hardware_info.h>
	hardwareinfo_set_prop(HARDWARE_ALSPS, "ap3426");
	ap3xx6_init_flag = 0;
	APS_LOG("%s: OK\n", __func__);
	return 0;
