...

{
	...
	switch(cmd)
	{
		...
		//---------------------------------
		case GSENSOR_SET_ACC_MODE://wt add start
			...
			if(copy_from_user(&enable, data, sizeof(enable)))
				...
				if(enable == 1)
					err =BMA2XX_SetPowerMode(client,BMA_NORMAL_MODE);
				else
					err = BMA2XX_SetPowerMode(client,BMA_SUSPEND_MODE);
				if (err)
