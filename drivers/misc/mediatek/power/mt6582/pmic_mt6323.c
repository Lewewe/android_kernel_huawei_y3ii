...
void PMIC_IMM_PollingAuxadcChannel(void)
{
	kal_uint32 ret=0;
	 ...
	 //pr_notice( "[PMIC_IMM_PollingAuxadcChannel] before:%d ",upmu_get_rg_adc_deci_gdly());
	...
	//pr_notice( "[PMIC_IMM_PollingAuxadcChannel] after:%d ",upmu_get_rg_adc_deci_gdly());
}
...
int PMIC_IMM_GetOneChannelValue(int dwChannel, int deCount, int trimd)
{
    ...
    */
	...
	do
	{
	    ...
	    {
	        //debug
	        pr_notice( "[AUXADC] u4channel[%d]=%d.\n",
	            dwChannel, ret_data);
	    }
    ...
    {
        //debug
        pr_notice( "[AUXADC] adc_result_temp=%d, adc_result=%d, r_val_temp=%d.\n",
...
int pmic_get_buck_current(int avg_times)
{
    ...
    pr_notice( "[pmic_get_buck_current] before meter, Reg[0x%x]=0x%x, Reg[0x%x]=0x%x, Reg[0x%x]=0x%x\n", 
    ...
    pr_notice( "[pmic_get_buck_current] after meter, Reg[0x%x]=0x%x, Reg[0x%x]=0x%x, Reg[0x%x]=0x%x\n", 
        0x200, upmu_get_reg_value(0x200), 
        0x758, upmu_get_reg_value(0x758), 
        0x76E, upmu_get_reg_value(0x76E)
        );
...
void watchdog_int_handler(void)
{
    ...
    pr_notice( "[watchdog_int_handler]....\n");
...
void pwrkey_int_handler(void)
{
		...
		if(upmu_get_pwrkey_deb()==1)
    {
		...
		if(g_boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT && timer_pre != 0)
		{
				...
				pr_notice( "[pmic_thread_kthread] timer_pos = %ld, timer_pre = %ld, timer_pos-timer_pre = %ld, long_pwrkey_press = %d\r\n",timer_pos, timer_pre, timer_pos-timer_pre, long_pwrkey_press);
				if(long_pwrkey_press)   //500ms
				{
					pr_notice( "[pmic_thread_kthread] Power Key Pressed during kernel power off charging, reboot OS\r\n");
					arch_reset(0, NULL);
				}
		}
#endif
...
void vbaton_undet_int_handler(void)
{
    ...
    pr_notice( "[vbaton_undet_int_handler]....\n");
...
void bvalid_det_int_handler(void)
{
    ...
    pr_notice( "[bvalid_det_int_handler]....\n");
...
void chrdet_int_handler(void)
{
    ...
    if (!upmu_get_rgs_chrdet())
    {
        ...
        {
            //pr_notice( "[chrdet_int_handler] Unplug Charger/USB In Kernel Power Off Charging Mode!  Shutdown OS!\r\n");
            //mt_power_off();
        }
    }
	pr_notice( "[chrdet_int_handler] upmu_get_rgs_chrdet = %d\n", upmu_get_rgs_chrdet());
#endif
    do_chrdet_int_task();
...
void fchr_key_int_handler(void)
{
    ...
    pr_notice( "[fchr_key_int_handler]....\n");
...
void vproc_oc_int_handler(void)
{
    ...
    pr_notice( "[vproc_oc_int_handler]....\n");
...
static int pmic_thread_kthread(void *x)
{
    ...
    pr_notice( "[pmic_thread_kthread] enter\n");

    /* Run on a process content */
    while (1) {
        mutex_lock(&pmic_mutex);

        pr_notice( "[pmic_thread_kthread] running\n");

        //--------------------------------------------------------------------------------
        ret=pmic_read_interface(INT_STATUS0,(&int_status_val_0),0xFFFF,0x0);
        pr_notice( "[INT] int_status_val_0=0x%x\n", int_status_val_0);
        ...
        pr_notice( "[INT] int_status_val_1=0x%x\n", int_status_val_1);
        ...
        ret=pmic_read_interface(INT_STATUS0,(&int_status_val_0),0xFFFF,0x0);
        pr_notice( "[INT] after ,int_status_val_0=0x%x\n", int_status_val_0);

        ret=pmic_read_interface(INT_STATUS1,(&int_status_val_1),0xFFFF,0x0);
        pr_notice( "[INT] after ,int_status_val_1=0x%x\n", int_status_val_1);
...
{
    pr_notice( "[mt6323_pmic_eint_irq] receive interrupt\n");
...
{
    pr_notice( "[PMIC_EINT_SETTING] start: CUST_EINT_MT6323_PMIC_NUM=%d\n",CUST_EINT_MT6323_PMIC_NUM);
    ...
    //pr_notice( "[CUST_EINT] GPIO37=GPIO_MODE_01 for EINT3 usage\n");
    ...
    pr_notice( "[CUST_EINT] CUST_EINT_MT6323_PMIC_NUM=%d\n", CUST_EINT_MT6323_PMIC_NUM);
    pr_notice( "[CUST_EINT] CUST_EINT_MT6323_PMIC_DEBOUNCE_CN=%d\n", CUST_EINT_MT6323_PMIC_DEBOUNCE_CN);
    //pr_notice( "[CUST_EINT] CUST_EINT_MT6323_PMIC_POLARITY=%d\n", CUST_EINT_MT6323_PMIC_POLARITY);
    //pr_notice( "[CUST_EINT] CUST_EINT_MT6323_PMIC_SENSITIVE=%d\n", CUST_EINT_MT6323_PMIC_SENSITIVE);
    pr_notice( "[CUST_EINT] CUST_EINT_MT6323_PMIC_TYPE=%d\n", CUST_EINT_MT6323_PMIC_TYPE);
    pr_notice( "[CUST_EINT] CUST_EINT_MT6323_PMIC_DEBOUNCE_EN=%d\n", CUST_EINT_MT6323_PMIC_DEBOUNCE_EN);
...
U32 pmic_read_interface (U32 RegNum, U32 *val, U32 MASK, U32 SHIFT)
{
    ...
    if(return_value!=0)
    {
        pr_notice( "[pmic_read_interface] Reg[%x]= pmic_wrap read data fail\n", RegNum);
    ...
    *val = (pmic6323_reg >> SHIFT);
    ...
    mutex_unlock(&pmic_access_mutex);
#else
    pr_notice( "[pmic_read_interface] Can not access HW PMIC\n");
#endif
...
U32 pmic_config_interface (U32 RegNum, U32 val, U32 MASK, U32 SHIFT)
{
    ...
    if(return_value!=0)
    {
        pr_notice( "[pmic_config_interface] Reg[%x]= pmic_wrap read data fail\n", RegNum);
        mutex_unlock(&pmic_access_mutex);
        return return_value;
    }
    ...
    if(return_value!=0)
    {
        pr_notice( "[pmic_config_interface] Reg[%x]= pmic_wrap write data fail\n", RegNum);
        mutex_unlock(&pmic_access_mutex);
        return return_value;
    }
    //pr_debug( "[pmic_config_interface] Reg[%x]=0x%x\n", RegNum, pmic6323_reg);
    ...
    mutex_unlock(&pmic_access_mutex);
#else
    pr_notice( "[pmic_config_interface] Can not access HW PMIC\n");
#endif
...
U32 pmic_read_interface_nolock (U32 RegNum, U32 *val, U32 MASK, U32 SHIFT)
{
    ...
    if(return_value!=0)
    {
        pr_notice( "[pmic_read_interface_nolock] Reg[%x]= pmic_wrap read data fail\n", RegNum);
        return return_value;
    }
...
#else
    pr_notice( "[pmic_read_interface_nolock] Can not access HW PMIC\n");
#endif
...
U32 pmic_config_interface_nolock (U32 RegNum, U32 val, U32 MASK, U32 SHIFT)
{
    ...
    if(return_value!=0)
    {
        pr_notice( "[pmic_config_interface_nolock] Reg[%x]= pmic_wrap read data fail\n", RegNum);
        return return_value;
    }
...
#else
    pr_notice( "[pmic_config_interface_nolock] Can not access HW PMIC\n");
#endif
...
static ssize_t store_pmic_access(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    if(buf != NULL && size != 0)
    {
        pr_notice( "[store_pmic_access] buf is %s and size is %zu \n",buf,size);
...
{
    pr_notice( "[dct_pmic_VPROC_enable] %d\n", dctEnable);
...
{
    pr_notice( "[dct_pmic_VSYS_enable] %d\n", dctEnable);
...
{
    pr_notice( "[dct_pmic_VPA_enable] %d\n", dctEnable);
...
{
    pr_notice( "[dct_pmic_VIO28_enable] %d\n", dctEnable);
...
{
    pr_notice( "[dct_pmic_VUSB_enable] %d\n", dctEnable);
...
{
    pr_notice( "[dct_pmic_VMC_enable] %d\n", dctEnable);
...
{
    pr_notice( "[dct_pmic_VMCH_enable] %d\n", dctEnable);
...
{
    pr_notice( "[dct_pmic_VEMC_3V3_enable] %d\n", dctEnable);
...
{
    pr_notice( "[dct_pmic_VGP1_enable] %d\n", dctEnable);
...
{
    pr_notice( "[dct_pmic_VGP2_enable] %d\n", dctEnable);
...
{
    pr_notice( "[dct_pmic_VGP3_enable] %d\n", dctEnable);
...
{
    pr_notice( "[dct_pmic_VCN18_enable] %d\n", dctEnable);
...
{
    pr_notice( "[dct_pmic_VSIM1_enable] %d\n", dctEnable);
...
{
    pr_notice( "[dct_pmic_VSIM2_enable] %d\n", dctEnable);
...
{
    pr_notice( "[dct_pmic_VRTC_enable] %d\n", dctEnable);
...
{
    pr_notice( "[dct_pmic_VCAM_AF_enable] %d\n", dctEnable);
...
{
    pr_notice( "[dct_pmic_VIBR_enable] %d\n", dctEnable);
...
{
    pr_notice( "[dct_pmic_VM_enable] %d\n", dctEnable);
...
{
    pr_notice( "[dct_pmic_VRF18_enable] %d\n", dctEnable);
...
{
    pr_notice( "[dct_pmic_VIO18_enable] %d\n", dctEnable);
...
{
    pr_notice( "[dct_pmic_VCAMD_enable] %d\n", dctEnable);
...
{
    pr_notice( "[dct_pmic_VCAM_IO_enable] %d\n", dctEnable);
...
{
    pr_notice( "[dct_pmic_VTCXO_enable] %d\n", dctEnable);
...
{
    pr_notice( "[dct_pmic_VA_enable] %d\n", dctEnable);
...
{
    pr_notice( "[dct_pmic_VCAMA_enable] %d\n", dctEnable);
...
{
    pr_notice( "[dct_pmic_VCN28_enable_bt] %d\n", dctEnable);
...
{
    pr_notice( "[dct_pmic_VCN28_enable_wifi] %d\n", dctEnable);
...
{
    pr_notice( "[dct_pmic_VCN28_enable] %d\n", dctEnable);
...
{
    pr_notice( "****[dct_pmic_VMC_sel] value=%d \n", volt);
    ...
    if(volt == VOL_DEFAULT)     {upmu_set_rg_vmc_vosel(1);}
        ...
        pr_notice( "Error Setting %d. DO nothing.\r\n", volt);
    }
}

void dct_pmic_VMCH_sel(kal_uint32 volt)
{
    pr_notice( "****[dct_pmic_VMCH_sel] value=%d \n", volt);
...
{
    pr_notice( "****[dct_pmic_VEMC_3V3_sel] value=%d \n", volt);
...
{
    pr_notice( "****[dct_pmic_VGP1_sel] value=%d \n", volt);
...
{
    pr_notice( "****[dct_pmic_VGP2_sel] value=%d \n", volt);
...
{
    pr_notice( "****[dct_pmic_VGP3_sel] value=%d \n", volt);
...
{
    pr_notice( "****[dct_pmic_VCAM_AF_sel] value=%d \n", volt);
...
{
    pr_notice( "****[dct_pmic_VSIM1_sel] value=%d \n", volt);
...
{
    pr_notice( "****[dct_pmic_VSIM2_sel] value=%d \n", volt);
...
{
    pr_notice( "****[dct_pmic_VIBR_sel] value=%d \n", volt);
...
{
    pr_notice( "****[dct_pmic_VM_sel] value=%d \n", volt);
...
{
    pr_notice( "****[dct_pmic_VCAMD_sel] value=%d \n", volt);
...
{
    pr_notice( "****[dct_pmic_VCAMA_sel] value=%d \n", volt);
...
{
    pr_notice( "****[dct_pmic_VCN33_sel] value=%d \n", volt);
...
static ssize_t show_BUCK_VPROC_STATUS(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    //pr_debug( "[EM] BUCK_VPROC_STATUS : %d\n", ret_value);
    return sprintf(buf, "%u\n", ret_value);
}
...
static ssize_t show_BUCK_VSYS_STATUS(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    //pr_debug( "[EM] BUCK_VSYS_STATUS : %d\n", ret_value);
    return sprintf(buf, "%u\n", ret_value);
}
...
static ssize_t show_LDO_VTCXO_STATUS(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    //pr_debug( "[EM] LDO_VTCXO_STATUS : %d\n", ret_value);
    return sprintf(buf, "%u\n", ret_value);
}
...
static ssize_t show_LDO_VCN28_STATUS(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    //pr_debug( "[EM] LDO_VCN28_STATUS : %d\n", ret_value);
    return sprintf(buf, "%u\n", ret_value);
}
...
static ssize_t show_LDO_VCN33_STATUS(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    //pr_debug( "[EM] LDO_VCN33_STATUS : %d\n", ret_value);
    return sprintf(buf, "%u\n", ret_value);
}
...
static ssize_t show_LDO_VEMC_3V3_STATUS(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    //pr_debug( "[EM] LDO_VEMC_3V3_STATUS : %d\n", ret_value);
    return sprintf(buf, "%u\n", ret_value);
}
...
static ssize_t show_LDO_VCN_1V8_STATUS(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    //pr_debug( "[EM] LDO_VCN_1V8_STATUS : %d\n", ret_value);
    return sprintf(buf, "%u\n", ret_value);
}
...
static ssize_t show_LDO_VRF18_STATUS(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    //pr_debug( "[EM] LDO_VRF18_STATUS : %d\n", ret_value);
    return sprintf(buf, "%u\n", ret_value);
}
...
static ssize_t show_LDO_VIO18_STATUS(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    //pr_debug( "[EM] LDO_VIO18_STATUS : %d\n", ret_value);
    return sprintf(buf, "%u\n", ret_value);
}
...
static ssize_t show_BUCK_VPROC_VOLTAGE(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    //pr_debug( "[EM] BUCK_VPROC_VOLTAGE : %d\n", ret_value);
    return sprintf(buf, "%u\n", ret_value);
}
...
static ssize_t show_BUCK_VSYS_VOLTAGE(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    //pr_debug( "[EM] BUCK_VSYS_VOLTAGE : %d\n", ret_value);
    return sprintf(buf, "%u\n", ret_value);
}
...
static ssize_t show_LDO_VEMC_3V3_VOLTAGE(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    //pr_debug( "[EM] LDO_VEMC_3V3_VOLTAGE : %d\n", ret_value);
    return sprintf(buf, "%u\n", ret_value);
}
...
static ssize_t show_LDO_VCN33_VOLTAGE(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    //pr_debug( "[EM] LDO_VCN33_VOLTAGE : %d\n", ret_value);
    return sprintf(buf, "%u\n", ret_value);
}
...
static ssize_t show_BUCK_CURRENT_METER(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    //pr_debug( "[EM] BUCK_CURRENT_METER : %d\n", ret_value);
    return sprintf(buf, "%u\n", ret_value);
}
...
void pmic_ldo_enable(MT65XX_POWER powerId, kal_bool powerEnable)
{
    ...
    {
        pr_notice( "[pmic_ldo_enable] UnKnown powerId (%d)\n", powerId);
    }
...
void pmic_ldo_vol_sel(MT65XX_POWER powerId, MT65XX_POWER_VOLTAGE powerVolt)
{
    ...
    if(     powerId == MT6323_POWER_LDO_VIO28)      { pr_notice( "[dct_pmic_VIO28_enable] No vlotage can setting!\n"); }
    else if(powerId == MT6323_POWER_LDO_VUSB)       { pr_notice( "[dct_pmic_VUSB_enable] No vlotage can setting!\n"); }
    ...
    else if(powerId == MT6323_POWER_LDO_VCN_1V8)        {pr_notice( "[dct_pmic_VCN_1V8_enable] No vlotage can setting!\n");}
    ...
    else if(powerId == MT6323_POWER_LDO_VRTC)        { pr_notice( "[dct_pmic_VRTC_enable] No vlotage can setting!\n");}
    ...
    else if(powerId == MT6323_POWER_LDO_VRF18)        { pr_notice( "[dct_pmic_VRF18_enable] No vlotage can setting!\n");}
    else if(powerId == MT6323_POWER_LDO_VIO18)        { pr_notice( "[dct_pmic_VIO18_enable] No vlotage can setting!\n");}
    else if(powerId == MT6323_POWER_LDO_VCAMD)        { dct_pmic_VCAMD_sel(powerVolt); }
    else if(powerId == MT6323_POWER_LDO_VCAM_IO)    { pr_notice( "[dct_pmic_VCAM_IO_enable] No vlotage can setting!\n");}
    else if(powerId == MT6323_POWER_LDO_VTCXO)        { pr_notice( "[dct_pmic_VTCXO_enable] No vlotage can setting!\n");}
    else if(powerId == MT6323_POWER_LDO_VA)         { pr_notice( "[dct_pmic_VA_enable] No vlotage can setting!\n"); }
    ...
    else if(powerId == MT6323_POWER_LDO_VCN28)      { pr_notice( "[dct_pmic_VCN28_enable] No vlotage can setting!\n"); }
    else
    {
        pr_notice( "[pmic_ldo_ldo_vol_sel] UnKnown powerId (%d)\n", powerId);
    }
...
void pmic_debug_init(void)
{
    ...
    if (!mt_pmic_dir) {
        pr_notice( "fail to mkdir /proc/mt_pmic\n" );
        return;
    }
    ...
    pr_notice( "proc_create pmic_debug_proc_fops\n" );
#endif
}
...
static int pmic_mt6323_probe(struct platform_device *dev)
{
    ...
    pr_notice( "******** MT6323 pmic driver probe!! ********\n" );


    //get PMIC CID
    ret_val=upmu_get_cid();
    pr_notice( "MT6323 PMIC CID=0x%x\n", ret_val );
    ...
    pr_notice( "[PMIC_INIT_SETTING_V1] Done\n");
    PMIC_CUSTOM_SETTING_V1();
    pr_notice( "[PMIC_CUSTOM_SETTING_V1] Done\n");
    ...
    //PMIC Interrupt Service
    PMIC_EINT_SETTING();
    pr_notice( "[PMIC_EINT_SETTING] Done\n");

    kthread_run(pmic_thread_kthread, NULL, "pmic_thread_kthread");
    pr_notice( "[pmic_thread_kthread] Done\n");
    ...
    pmic_debug_init();
    pr_notice( "[PMIC] pmic_debug_init : done.\n" );
    ...
    pr_notice( "[PMIC] VTCXO control by SRCLKEM due to CONFIG_MTK_ENABLE_MD5, Reg[0x402]=0x%x\n",
...
{
    pr_notice( "******** MT6323 pmic driver remove!! ********\n" );
...
{
    pr_notice( "******** MT6323 pmic driver resume!! ********\n" );
...
{
	pr_notice( "******** MT6323 pmic driver early suspend!! ********\n" );
	upmu_set_rg_vref18_enb(0);
	//upmu_set_rg_adc_deci_gdly(0);
	upmu_set_rg_clksq_en_aux(1);
	upmu_set_rg_aud26m_div4_ck_pdn(0);
...
{
	pr_notice( "******** MT6323 pmic driver early resume!! ********\n" );
	upmu_set_rg_vref18_enb(0);
	//upmu_set_rg_adc_deci_gdly(0);
	upmu_set_rg_clksq_en_aux(1);
	upmu_set_rg_aud26m_div4_ck_pdn(0);
...
static int __init pmic_mt6323_init(void)
{
    ...
    if (ret) {
        pr_notice( "****[pmic_mt6323_init] Unable to device register(%d)\n", ret);
        return ret;
    }
    ret = platform_driver_register(&pmic_mt6323_driver);
    if (ret) {
        pr_notice( "****[pmic_mt6323_init] Unable to register driver (%d)\n", ret);
        return ret;
    }
    ...
    pr_notice( "****[pmic_mt6323_init] Initialization : DONE !!\n");
