...
static kal_uint32 mt_get_board_type(void)
{
	...
	/*
	 ...
	 battery_log(BAT_LOG_CRTI, "[Kernel] Board type is %s\n", (board_type == MT_BOARD_EVB) ? "EVB" : "PHONE");
	 return board_type;
#else
	 return MT_BOARD_EVB;
#endif
...
static kal_uint32 charging_get_csdac_value(void)
{
	...
	battery_log(BAT_LOG_CRTI, "tempC=%d,tempA=%d,tempB=%d, csdac=%d\n", tempC,tempA,tempB,sum);
...
kal_bool charging_type_detection_done(void)
 ...
 static kal_uint32 charging_get_is_pcm_timer_trigger(void *data)
 {
     ...
     battery_log(BAT_LOG_CRTI, "[Early porting] no slp_get_wake_reason at FPGA\n");
 #else
