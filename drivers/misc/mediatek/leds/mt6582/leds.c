	...
	*/	
	...
	switch (cust->mode) {
		...
		case MT65XX_LED_MODE_CUST:
			LEDS_DEBUG("brightness_set_cust:go led cust mode!!!!!\n");
			if (strcmp(cust->name,"red") == 0) {
				((cust_led_brightness)(cust->data))(level, 0, 0, 0, 0);
			} else if (strcmp(cust->name,"green") == 0) {
				((cust_led_brightness)(cust->data))(0, level, 0, 0, 0);
			} else if (strcmp(cust->name,"blue") == 0) {
				((cust_led_brightness)(cust->data))(0, 0, level, 0, 0);
			}
			return 0;
...
void mt_mt65xx_led_work(struct work_struct *work)
{
	...
	LEDS_DEBUG("[LED]cust.name=%s,level=%d,delay_on=%d,delay_off=%d\n",
		led_data->cust.name, led_data->level, led_data->delay_on, led_data->delay_off);
	mutex_lock(&leds_mutex);
	if (((g_boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT) ||\
			(g_boot_mode == LOW_POWER_OFF_CHARGING_BOOT)) && \
			((led_data->delay_on) || (led_data->delay_off))) {
		//FIX ME: do nothing here
		//mt_mt65xx_blink_set(&led_data->cdev, led_data->delay_on, led_data->delay_off);
	} else {
			     ...
			     unsigned long *delay_off)
{
	...
	if (*delay_on != led_data->delay_on || *delay_off != led_data->delay_off) {
		...
		if (led_data->delay_on && led_data->delay_off) { // enable blink
			...
			} else if (led_data->cust.mode == MT65XX_LED_MODE_CUST) {
				LEDS_DEBUG("mt_mt65xx_blink_set:go led cust mode on!!!!!\n");
				if (strcmp(led_data->cust.name,"red") == 0) {
					((cust_led_brightness)(led_data->cust.data))(255, 0, 0, led_data->delay_on, led_data->delay_off);
				} else if (strcmp(led_data->cust.name,"green") == 0) {
					((cust_led_brightness)(led_data->cust.data))(0, 255, 0, led_data->delay_on, led_data->delay_off);
				} else if (strcmp(led_data->cust.name,"blue") == 0) {
					((cust_led_brightness)(led_data->cust.data))(0, 0, 255, led_data->delay_on, led_data->delay_off);
				}
				return 0;
			}
				...
				|| led_data->cust.data == MT65XX_LED_PMIC_NLED_ISINK3))
			{
				...
				LEDS_DEBUG("mt_mt65xx_blink_set:go led cust mode off!!!!!\n");
				if (strcmp(led_data->cust.name,"red") == 0) {
					((cust_led_brightness)(led_data->cust.data))(0, 0, 0, 0, 0);
				} else if (strcmp(led_data->cust.name,"green") == 0) {
					((cust_led_brightness)(led_data->cust.data))(0, 0, 0, 0, 0);
				} else if (strcmp(led_data->cust.name,"blue") == 0) {
					((cust_led_brightness)(led_data->cust.data))(0, 0, 0, 0, 0);
