...
static long hardwareinfo_ioctl(struct file *file, unsigned int cmd,unsigned long arg)
{
	...
	case HARDWARE_MODEM_BAND_GET:
		...
		break;
#endif
	case HARDWARE_BATTERY_GET:
		hardwareinfo_set_prop(HARDWARE_BATTERY, "Linear charger");
			hardwareinfo_num = HARDWARE_BATTERY;
			break;
	case HARDWARE_SPEAKER_PA_GET:
		hardwareinfo_set_prop(HARDWARE_SPEAKER_PA, "AW8738");
			hardwareinfo_num = HARDWARE_SPEAKER_PA;
			break;
