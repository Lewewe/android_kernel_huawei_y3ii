...
static s32 gt91xx_config_write_proc(struct file *filp, const char __user *buff, unsigned long len, void *data);
static s32 gt91xx_config_read_proc(char *page, char **start, off_t off, int count, int *eof, void *data);
static  const struct file_operations  proc_goodix_config_fops = 
{
...
static s32 gtp_init_panel(struct i2c_client *client)
{
...
#endif
    GTP_DEBUG("len1=%d,len2=%d,len3=%d,len4=%d,len5=%d,len6=%d", 
    ...
    cfg_len = cfg_info_len[rd_cfg_buf[GTP_ADDR_LENGTH]];
    GTP_INFO("SENSOR ID:%d,cfg_len:%d", rd_cfg_buf[GTP_ADDR_LENGTH],cfg_len);
    Module_ID = rd_cfg_buf[GTP_ADDR_LENGTH];
...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    gt91xx_config_proc = proc_create( GT91XX_CONFIG_PROC_FILE, 0664, NULL,&proc_goodix_config_fops);
    ...
    */
    ...
    tpd_load_status = 1;
		...
		if(1 == Module_ID){
			#if GTP_SHORTOPEN_TEST
				max_limit_value = 3500;     // screen max limit
				min_limit_value = 1300;     // screen min limit
				max_limit_key = 1500;       // key_val max limit
				min_limit_key = 500;        // key_val min limit
			#endif
			//hardwareinfo_set_prop(HARDWARE_TP, "GT9xx_oufeiguang");
		}else if(2 == Module_ID){
			#if GTP_SHORTOPEN_TEST
				max_limit_value = 3500;     // screen max limit
				min_limit_value = 1300;     // screen min limit
				...
				min_limit_key = 500;        // key_val min limit
			#endif
		 //	hardwareinfo_set_prop(HARDWARE_TP, "GT9xx_laibao");
		}else{
			 #if GTP_SHORTOPEN_TEST
				max_limit_value = 3500;     // screen max limit
				min_limit_value = 1300;     // screen min limit
...
{
    s8 ret = -1;
#if 1 // !GTP_POWER_CTRL_SLEEP // I2C cmd sleep ,changed by fae ,2013/05/21
    s8 retry = 0;
...
#if 0 // GTP_POWER_CTRL_SLEEP  // eint wakeup !! ,changed by fae ,2013/05/21
