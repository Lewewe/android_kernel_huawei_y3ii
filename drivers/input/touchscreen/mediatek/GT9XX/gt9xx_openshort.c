...
u16 max_limit_value = 3182;     // screen max limit
u16 min_limit_value = 1335;     // screen min limit
u16 max_limit_key = 1103;       // key_val max limit
u16 min_limit_key = 532;        // key_val min limit
...
static ssize_t gtp_sysfs_shorttest_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	...
	printk("gtp_sysfs_shorttest_show result=%x",result);
	sprintf(&buf[0], "result=%d", result);
    RsltIndex = 0;
    return len;
}
...
static ssize_t gtp_sysfs_opentest_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	...
	printk("gtp_sysfs_opentest_show result=%x",result);
	sprintf(&buf[0], "result=%d", result);
    RsltIndex = 0;
    return len;
}
