...
extern ItoTestResult_e _DrvMpTestItoOpenTest(void);
extern  ItoTestResult_e _DrvMpTestItoShortTest(void);
...
static struct proc_dir_entry *ctp_test = NULL;
...
//------------------------------------------------------------------------------//
ssize_t ctp_open_test_read(struct file *pFile, char __user *pBuffer, size_t nCount, loff_t *pPos)
{
...
for(i=0;i<3;i++)
{
    if(ITO_TEST_OK == _DrvMpTestItoOpenTest()){
		opentest = 1;
		break;
    }else{
		opentest = 0;
...
for(i=0;i<3;i++)
{ 
   if(ITO_TEST_OK == _DrvMpTestItoShortTest()){
		closetest = 1;
		break;
    }else{
		closetest = 0;
   ...
   if(closetest && opentest ){
    ...
    nLength = sprintf(pBuffer, "result=%d\n", result);
...
ssize_t ctp_open_test_write(struct file *pFile, const char __user *pBuffer, size_t nCount, loff_t *pPos)  
{
    DBG("enter open test write");
...
static const struct file_operations ctp_open_test = { 
    .read = ctp_open_test_read,
    .write = ctp_open_test_write,
};

ssize_t ctp_short_test_read(struct file *pFile, char __user *pBuffer, size_t nCount, loff_t *pPos)
{
    int result = 0;
	   int nLength = 0;
    // If file position is non-zero, then assume the string has been read and indicate there is no more data to be read.
    ...
    nLength = sprintf(pBuffer, "notest\n");
...
ssize_t ctp_short_test_write(struct file *pFile, const char __user *pBuffer, size_t nCount, loff_t *pPos)  
{
	DBG("enter short test write");
//	  g_ChipType = DrvIcFwLyrGetChipType();
	return nCount;
}

static const struct file_operations ctp_short_test = { 
    .read = ctp_short_test_read,
    .write = ctp_short_test_write,
};
...
s32 DrvMainTouchDeviceInitialize(void)
{
	...
	if(g_ChipType == 0)
	{
	      DBG("*** I2C READ CHIP ID FAIL ***\n");
	      return -ENODEV;
	}
    }

#if CTP_ADD_HW_INFO
		u16 major_num = 0, minor_num = 0;
		u8 *version = NULL;
		DrvFwCtrlGetCustomerFirmwareVersion(&major_num, &minor_num, &version);
		
		if(major_num == 0x01){
			sprintf(tp_string_version,"msg2238,Yejii,IC VERSION:%2x_%2x", major_num, minor_num);
		}
		else if(major_num == 0x09){
			sprintf(tp_string_version,"msg2238,huaruichuan,IC VERSION:%2x_%2x", major_num, minor_num);
		}else  if(major_num == 0x10){
			sprintf(tp_string_version,"msg2238,helitai,IC VERSION:%2x_%2x", major_num, minor_num);
		}else{
			sprintf(tp_string_version,"msg2238,unknwon,IC VERSION:%2x_%2x", major_num, minor_num);
		}
...
static s32 _DrvMainCreateProcfsDirEntry(void)
{
	...
	ctp_test = proc_mkdir("ctp_test", NULL);
	if (NULL == proc_create("opentest", PROCFS_AUTHORITY, ctp_test, &ctp_open_test))
	{
	...
	if (NULL == proc_create("shorttest", PROCFS_AUTHORITY, ctp_test, &ctp_short_test))
	{
	    DBG("Failed to create procfs file node(%s)!\n", "shorttest");
	}   
	else 
