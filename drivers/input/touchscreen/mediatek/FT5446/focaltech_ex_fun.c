...
int fts_ctpm_fw_upgrade_with_i_file(struct i2c_client *client)
{
	...
	int fw_len = 0;
        ...
        if(origin_chip_id_value == 0x51){//ofilm white
...
u8 fts_ctpm_update_project_setting(struct i2c_client *client)
{
	...
	uc_panel_factory_id = 0x5a;
       ...
       FT_CTP_DEBUG("Step 1:Reset  CTPM\n");
	/*Step 1:Reset  CTPM
	 *write 0xaa to register 0xfc
	 */
	fts_get_upgrade_array();
	FT_CTP_DEBUG("fts_updateinfo_curr.CHIP_ID:0x%x, fts_updateinfo_curr.upgrade_id_1:0x%x, fts_updateinfo_curr.upgrade_id_2:0x%x\n",fts_updateinfo_curr.CHIP_ID,fts_updateinfo_curr.upgrade_id_1,fts_updateinfo_curr.upgrade_id_2);
	...
	FT_CTP_DEBUG("Step 2:Enter upgrade mode\n");
	...
	FT_CTP_DEBUG("Step 3:check READ-ID\n");
	...
	{
	        FT_CTP_DEBUG("Step 3:check READ-ID Fail!ID1 = 0x%x,ID2 = 0x%x\n",reg_val[0], reg_val[1]);
		return -EIO;
	}

	auc_i2c_write_buf[0] = 0xcd;
	fts_i2c_Read(client, auc_i2c_write_buf, 1, reg_val, 1);
	FT_CTP_DEBUG( "bootloader version = 0x%x\n", reg_val[0]);

	/*--------- read current project setting  ---------- */
	/*set read start address */
	FT_CTP_DEBUG("read current project setting \n");
	buf[0] = 0x3;
	buf[1] = 0x0;
	buf[2] = 0x07;
	buf[3] = 0xB0;
	...
	FT_CTP_DEBUG( "[FTS] old setting: uc_i2c_addr = 0x%x,\
			uc_io_voltage = %d, uc_panel_factory_id = 0x%x\n",
			buf[0], buf[2], buf[4]);

	/********* reset the new FW***********************/
	FT_CTP_DEBUG("reset the new FW \n");
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	//ofilm , id=0x51
	if((uc_reg_value != 0x51)||(uc_tp_fm_ver > 0x20)){
		origin_chip_id_value = fts_ctpm_update_project_setting(client);
		FT_CTP_DEBUG("force read id:0x%x", origin_chip_id_value);
		if( origin_chip_id_value == 0x51){
			is_ic_fw_crashed = 1;
		}else{
			return  -EIO;
		}
...
int fts_5x36_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	u8  	bt_ecc;
	int	i_ret;
	int	fw_filenth = sizeof(CTPM_FW_OFILM_WHITE);

	if(CTPM_FW_OFILM_WHITE[fw_filenth-12] == 30)
	{
		is_5336_fwsize_30 = 1;
	}
	else 
...
int  fts_5x46_ctpm_fw_upgrade(struct i2c_client * client, u8* pbt_buf, u32 dw_lenth)
{
	...
	for (j = 0; j < packet_number; j++) {
		...
		FT_CTP_DEBUG("write bytes:0x%04x\n", (j+1) * FTS_PACKET_LENGTH);
