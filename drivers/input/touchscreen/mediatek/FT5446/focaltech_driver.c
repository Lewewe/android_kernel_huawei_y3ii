...
static int msg_dma_alloct()
{
	...
	if(!g_dma_buff_va)
	{
		...
		FT_CTP_DEBUG("[DMA][Debug] Allocate DMA I2C Buffer success!!!\n");
	}
...
void fts_get_upgrade_array(void)
{
	...
	{
		memcpy(&fts_updateinfo_curr, &fts_updateinfo[4], sizeof(struct Upgrade_Info));
	}
}
/************************************************************************
 * Name: tpd_down
...
#if RF_OPEN
	char rf[3] = "0\n";
static ssize_t  rf_proc_read(struct file *file, char __user *buf,size_t count, loff_t *ppos);
static ssize_t  rf_proc_write(struct file *file, char __user *buf,size_t count, loff_t *ppos);
static  const struct file_operations  proc_rf_file_fops = 
{
	  .write = rf_proc_write,
    .read = rf_proc_read,
    //.open = simple_open,
...
static ssize_t  rf_proc_read(struct file *file, char __user *buf,size_t count, loff_t *ppos)//char *page, char **start, off_t off, int count, int *eof, void *data)
{
	char *ptr = buf;
	...
	if(*ppos)
       ...
       *ppos += count;
        ...
        FT_CTP_DEBUG("rf read");
        return sprintf(ptr, rf);
}
static ssize_t  rf_proc_write(struct file *file, char __user *buf,size_t count, loff_t *ppos)
{    
	  int len = 0;
	  ...
	  if( count < len )
	   len = count;
	  if(copy_from_user(rf, buf, len))
	   return -EFAULT;
	  
	  rf[len] = 0;
	  return len;
}
static void create_rf_proc(void)
{
	struct proc_dir_entry  *proc_dir = NULL;
	struct proc_dir_entry  *proc_entry = NULL;
        
        proc_dir = proc_mkdir("rf_", NULL);
         if (NULL==proc_dir) 
    	  {
       	  		FT_CTP_DEBUG("creating  /proc/rf_/  failed\n");
   	   }      
   	    
    	 proc_entry = proc_create("rf", ITO_TEST_AUTHORITY, proc_dir,&proc_rf_file_fops);
			 if (NULL==proc_entry) 
    	  {
       	  	FT_CTP_DEBUG("creating  /proc/rf_/rf  failed\n");
   	  } 
...
static int touch_event_handler(void *unused)
{
	...
	if(rf[0] == '1')
	  {
	  	FT_CTP_DEBUG("write data  1 ");
		ret = fts_write_reg(global_i2c_client, 0xAD, 0x01);
		if (ret<0) 
		{
			FT_CTP_ERROR("[Focal][Touch] write value fail");
			//return ret;
	  ...
	  {
		FT_CTP_DEBUG("write data 0 ");
		ret = fts_write_reg(global_i2c_client, 0xAD, 0x00);
		if (ret<0) 
		{
			FT_CTP_ERROR("[Focal][Touch] write value fail");
			//return ret;
...
#if WT_ADD_CTP_INFO
       if(tp_info.vendor_id== 0x51){
		sprintf(tp_string_version,"FT6336,oufei ,fw_ver:0x%x", tp_info.fw_version);
	}else{
		sprintf(tp_string_version,"FT6336,unknwon,fw_ver:0x%x", tp_info.fw_version);
	}
 ...
 ***********************************************************************/
static void 
tpd_resume( struct early_suspend *h )
{
	int i=0,ret = 0;
