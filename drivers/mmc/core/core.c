...
extern unsigned int card_cid_manfid;
extern unsigned char g_emmc_ecsd_rev;
...
int mmc_can_discard(struct mmc_card *card)
{
	 ...
	 */
	if ((card_cid_manfid == VENDOR_HYNIX) && (g_emmc_ecsd_rev == 7)){
		printk("disable discard for Hynix emmc5.0\n");
		return 0;
	}
	if ((card->ext_csd.feature_support & MMC_DISCARD_FEATURE)
        && !(card->quirks & MMC_QUIRK_DISABLE_DISCARD))
        return 1;
