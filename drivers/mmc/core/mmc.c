...
unsigned char g_emmc_ecsd_rev = 0;
unsigned int card_cid_manfid = 0;
...
static int mmc_decode_cid(struct mmc_card *card)
{
	...
	card_cid_manfid = card->cid.manfid;
	return 0;
}
...
static int mmc_read_ext_csd(struct mmc_card *card, u8 *ext_csd)
{
	...
	if( (!strcmp(emmc_device_name,"H9TQ64A8GTCCUR_KUM"))||(!strcmp(emmc_device_name,"KMFNX0012M_B214"))){
		pr_warning("add new flash, the emmc_flash is %s, EXT_CSD revision is %d\n", emmc_device_name,card->ext_csd.rev);
	}else{
